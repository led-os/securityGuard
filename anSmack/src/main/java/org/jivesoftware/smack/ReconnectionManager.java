...
public class ReconnectionManager {
    ...
    private ReconnectionManager(AbstractXMPPConnection connection) {
        ...
        reconnectionRunnable = new Thread() {
            ...
            public void run() {
                ...
                if (connection == null) {
                    //huawei test;
                    //FLog.d("reconnectionRunnable connection == null");
                    return;
                }
                ...
                while (isReconnectionPossible(connection)) {
                    //huawei test;
                    //FLog.d("reconnectionRunnable isReconnectionPossible(connection)");
                    ...
                    while (isReconnectionPossible(connection) && remainingSeconds > 0) {
                        //huawei test;
                        //FLog.d("reconnectionRunnable remainingSeconds" + remainingSeconds);
                        try {
                            Thread.sleep(1000);
                            remainingSeconds--;
                            ...
                            //huawei test;
                            //FLog.d("reconnectionRunnable InterruptedException" + e.getMessage());
                    ...
                    try {
                        if (isReconnectionPossible(connection)) {
                            //huawei test;
                            //FLog.d("reconnectionRunnable start connect" );
                            connection.connect();
                        }
                        ...
                        if (!connection.isAuthenticated()) {
                            //huawei test;
                            //FLog.d("reconnectionRunnable !connection.isAuthenticated() start login" );
                            connection.login();
                        }
                        ...
                        //huawei test;
                        //FLog.d("reconnectionRunnable connect exception" + e.getMessage());
    ...
    public synchronized void reconnect() {
        ...
        if (connection == null) {
            ...
            //huawei test;
            //FLog.d("Connection is null, will not reconnect");
            return;
        }
        ...
        if (reconnectionThread != null && reconnectionThread.isAlive()) {
            //huawei test;
           // FLog.d("reconnectionThread != null && reconnectionThread.isAlive()");
            return;
        }
